...
const SendSMSModal: React.FC<SendSMSModalProps> = ({ job, onClose }) => {
  ...
  const generateMessageForPerson = (resource: any, assignment: Assignment) => {
    ...
    const location = job.location 
      ? `${job.location.address}\nhttps://www.openstreetmap.org/?mlat=${job.location.lat}&mlon=${job.location.lng}&zoom=15`
      : 'TBD';
...
Location: ${location}`;
  ...
  if (people.length === 0) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" style={{ zIndex: getZIndex('send-sms') }}>
