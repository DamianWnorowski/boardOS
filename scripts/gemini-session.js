...
class GeminiSession {
  ...
  async analyzeGit() {
    ...
    return await GeminiHelpers.getGitStatus();
  }

  async analyzeTests() {
    console.log('  🧪 Test results...');
    return await GeminiHelpers.getTestStatus();
  }

  async analyzeLinting() {
    console.log('  🔧 ESLint analysis...');
    return await GeminiHelpers.getLintStatus();
  }

  async analyzeServer() {
    console.log('  🌐 Server status...');
    return await GeminiHelpers.getServerStatus();
  }

  async analyzeDatabase() {
    console.log('  🗄️  Database status...');
    return await GeminiHelpers.getDatabaseStatus();
  }
  ...
  async getRecentActivity() {
    const recentCommits = await GeminiHelpers.execSafe('git log --oneline -5');
    const recentFiles = await GeminiHelpers.execSafe('find . -name "*.tsx" -o -name "*.ts" -mtime -1 | grep -v node_modules | head -10');
  ...
  async saveAnalysis(analysis) {
    // Save detailed JSON
    await GeminiHelpers.writeJsonSafe('CLAUDE_CONTEXT.json', analysis);
    
    // Save readable markdown
    const markdown = this.formatAsMarkdown(analysis);
    await GeminiHelpers.writeJsonSafe('CLAUDE_CONTEXT.md', markdown);
  ...
  formatAsMarkdown(analysis) {
    ...
    return `# BoardOS Gemini Context - ${GeminiHelpers.formatTimestamp()}
...
- **Previous Session**: ${analysis.previousSession ? GeminiHelpers.formatTimestamp(new Date(analysis.previousSession.timestamp)) : 'First session'}
...
---
*Generated by gemini-session.js at ${GeminiHelpers.formatTimestamp()}*
...
if (process.argv[1] && process.argv[1].endsWith('gemini-session.js')) {
  main();
}